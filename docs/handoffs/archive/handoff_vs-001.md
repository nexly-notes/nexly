...
- Selected as the next ready vertical slice from `docs/specs/mvp/vertical-slices.md:26-34` (VS-001 has no dependencies; user confirmed the pick)

**Success criteria (the slice demo, `docs/specs/mvp/vertical-slices.md:33`):**
...
- `src/types/note.ts:2` defines `NoteMode = "create" | "study"` with `create → "Lecture"` label — the `edit` value and Create/Edit/Study rename belong to **VS-002**, not this slice (`docs/specs/mvp/vertical-slices.md:150`); existing tests assert `"Lecture Mode"` (`src/components/editor/note-editor.test.tsx:32`) — do not rename
...
- Data model (`docs/specs/mvp/tech-specs.md:97-138`): `notes` (id uuid, user_id uuid, title text, content jsonb — Tiptap doc, word_count int, created_at, updated_at); `events` (id uuid, user_id uuid, type text, value int nullable, created_at) — events are **metadata only, never note content**; RLS per-user isolation, events insert-own
- FR-003 partial (`docs/specs/mvp/prd.md`): plain 30s save only — retry/backoff, amber unsaved badge, `beforeunload` warning, and localStorage recovery copy are **VS-003**
- FR-009 partial: sign-up requires beta ToS acceptance — didactic lecture notes only, PHI prohibited (`docs/specs/mvp/design.md:357-361`); runaway backstop is VS-008
- FR-012 partial: `events` schema + `note_created` / `note_opened` only; later events ride their slices
- Status bar shows "Auto-saved" when clean (`docs/specs/mvp/design.md:227-234`)
...
| `docs/specs/mvp/tech-specs.md`           | Data model, RLS, save design — source of truth                       |
| `docs/specs/mvp/vertical-slices.md:26-34`| VS-001 scope contract and demo script                                |
